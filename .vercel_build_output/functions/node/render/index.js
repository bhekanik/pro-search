...
var manifest = {
	assets: [
		...
		{ file: 'logo_192.png', size: 4500, type: 'image/png' },
		{ file: 'logo_512.png', size: 9541, type: 'image/png' },
		{ file: 'manifest.json', size: 370, type: 'application/json' },
		{ file: 'robots.txt', size: 66, type: 'text/plain' }
	],
...
var metadata_lookup = {
	'src/routes/__layout.svelte': {
		entry: 'pages/__layout.svelte-1d49a325.js',
		css: ['assets/pages/__layout.svelte-e8858dce.css'],
		js: ['pages/__layout.svelte-1d49a325.js', 'chunks/vendor-12abbdca.js'],
		styles: []
	},
	'.svelte-kit/build/components/error.svelte': {
		entry: 'error.svelte-08606be3.js',
		css: [],
		js: ['error.svelte-08606be3.js', 'chunks/vendor-12abbdca.js'],
		styles: []
	},
	'src/routes/index.svelte': {
		entry: 'pages/index.svelte-b0857921.js',
		css: [],
		js: ['pages/index.svelte-b0857921.js', 'chunks/vendor-12abbdca.js'],
		styles: []
	}
};
...
var _layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	return `<div class="${'relative dark:bg-gray-600'}"><div class="${'p-8 max-w-6xl mx-auto'}">${
...
var Filter = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	let { filter } = $$props;
	let { name } = $$props;
	let { noInput = false } = $$props;
	let { go = () => null } = $$props;
	if ($$props.filter === void 0 && $$bindings.filter && filter !== void 0)
		$$bindings.filter(filter);
	if ($$props.name === void 0 && $$bindings.name && name !== void 0) $$bindings.name(name);
	if ($$props.noInput === void 0 && $$bindings.noInput && noInput !== void 0)
		$$bindings.noInput(noInput);
	if ($$props.go === void 0 && $$bindings.go && go !== void 0) $$bindings.go(go);
	return `<main><div class="${'flex my-2'}"><label class="${'flex items-center gap-2'}"><input class="${'w-7 h-7'}" type="${'checkbox'}"${add_attribute(
		'name',
		`${name}-checkbox`,
		0
	)}${add_attribute('id', `${name}-checkbox`, 0)}${add_attribute('checked', filter.enabled, 1)}>
			${escape(name)}</label></div>

	${
		filter.enabled && !noInput
			? `<input ${
					!filter.enabled ? 'disabled' : ''
			  } class="${'w-full my-2 rounded-md text-lg p-4 border-2 border-gray-400 dark:border-gray-800'}" type="${'text'}" name="${'siteInput'}" id="${''}" placeholder="${'Site'}"${add_attribute(
					'value',
					filter.value,
					0
			  )}>`
...
var Routes = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	let searchTerm = '';
	let site = { enabled: false, value: '' };
	let title = { enabled: false };
	let fileType = { enabled: false, value: '' };
	let searchInput;
...
<main><h1 class="${'text-4xl text-center my-8'}">Pro-Search</h1>
	<div class="${'flex flex-col gap-2 md:flex-row'}"><select class="${'rounded-md text-lg p-4 border-2 border-gray-400 dark:border-gray-800'}" name="${'provider'}" id="${'provider'}"><option selected value="${'https://google.com/search?q='}">Google</option><option value="${'https://duckduckgo.com/?q='}">DuckDuckGo</option><option value="${'https://bing.com/search?q='}">Bing</option><option value="${'https://search.yahoo.com/search?q='}">Yahoo</option></select>
		<input class="${'w-full rounded-md text-lg p-4 border-2 border-gray-400 dark:border-gray-800'}" placeholder="${'Search'}" type="${'text'}" name="${''}" id="${''}"${add_attribute(
		'value',
		searchTerm,
		0
	)}${add_attribute('this', searchInput, 0)}>
